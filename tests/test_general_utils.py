...
"""
Does: End-to-end tests for general utils (load_config, log, nlp_utils) with cache, env, and NLP fallbacks.
Used by: CI sanity & recruiter-grade coverage of config loading, debug logging, and antonym/lemma helpers.
Returns: Asserts on typed errors, cache hits, env behavior, spaCy presence/absence, and WordNet antonyms.
"""
...
# ---------- Robust import helper with fallback shim for normalize_token ----------
def _ensure_src_on_path():
    src = Path("src").resolve()
    if src.is_dir() and str(src) not in sys.path:
        sys.path.insert(0, str(src))

def _install_normalize_token_shim():
    """
    Does: Provide a minimal module tree so that
          `from src.color_sentiment_extractor.extraction.general.token import normalize_token`
          can be imported even if the full package isn't available during tests.
    """
    pkg_names = [
        "src",
        "src.color_sentiment_extractor",
        "src.color_sentiment_extractor.extraction",
        "src.color_sentiment_extractor.extraction.general",
        "src.color_sentiment_extractor.extraction.general.token",
    ]
    for name in pkg_names:
        if name not in sys.modules:
            sys.modules[name] = ModuleType(name)

    # Minimal normalize_token: lowercases, trims, preserves hyphens when asked.
    ...
    sys.modules["src.color_sentiment_extractor.extraction.general.token"].normalize_token = _normalize_token
...
def _import_utils():
    ...
    LC = None
    for mod in lc_candidates:
        try:
            LC = importlib.import_module(mod)
            break
        except Exception:
            continue
    ...
    LOG = None
    for mod in log_candidates:
        try:
            LOG = importlib.import_module(mod)
            break
        except Exception:
            continue
    assert LOG is not None, "Could not import log module"

    # nlp_utils — may require shim if package layout differs
    nlp_candidates = [
        "color_sentiment_extractor.extraction.general.utils.nlp_utils",
        "Chatbot.extraction.general.utils.nlp_utils",
    ]
    NLP = None
    for mod in nlp_candidates:
        try:
            NLP = importlib.import_module(mod)
            break
        except Exception:
            continue
    if NLP is None:
        # Install shim for normalize_token and retry
...
DataDirNotFound = getattr(LC, "DataDirNotFound")
ConfigFileNotFound = getattr(LC, "ConfigFileNotFound")
ConfigParseError = getattr(LC, "ConfigParseError")
ConfigTypeError = getattr(LC, "ConfigTypeError")
load_config = getattr(LC, "load_config")
clear_config_cache = getattr(LC, "clear_config_cache")


# ---------- Fixtures ----------
@pytest.fixture
def tmp_data_dir(tmp_path, monkeypatch):
    """Does: Provide an isolated data/ dir and point loader via DATA_DIR. Returns: Path to data dir."""
    data = tmp_path / "data"
    data.mkdir()
...
def _reset_env_and_cache(monkeypatch):
    """Does: Reset debug topics and config cache between tests. Returns: None."""
...
def test_load_config_allow_comments_with_fake_json5(tmp_data_dir, monkeypatch):
    ...
    fake_json5 = types.SimpleNamespace(load=lambda f: {"a": 1, "b": 2})
...
def test_lemmatize_token_spacy_present_and_absent(monkeypatch):
    class _Tok:
        def __init__(self, text, lemma): self.text, self.lemma_ = text, lemma
    class _Doc(list): pass
    class _NLP:
        def __call__(self, txt): return _Doc([_Tok(txt, "LEMMA")])
